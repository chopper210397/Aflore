...
```{r plot1, echo=FALSE, message=FALSE, warning=FALSE,fig.height=10,fig.width=10}
...
plot1<-tickets %>% count(mes,Title) %>% ggplot(aes(x=reorder(Title,n),y=n,fill=mes))+geom_bar(stat = "identity",position = "dodge2")+
  coord_flip()+theme(legend.position = "none")+labs(x="",y="conteo")
ggplotly(plot1)
```
...
plot2<-ticket_comercial %>% count(mes,Title) %>% ggplot(aes(x=reorder(Title,n),y=n,fill=mes))+geom_bar(stat = "identity",position = "dodge2")+coord_flip()+labs(x="",y="conteo")+theme(legend.position = "none")
ggplotly(plot2)
