...
```{r reading}
contactoefectivo<-read_xlsx("~/Documentos/GitHub/LabJournalWebsite-master/docs/contactoefectivo1.xlsx")
...
borrower_profile %>% count(Status) %>% arrange(-n)
```
...
ggplotly(prestamosporgestion1 %>% filter(year==2022) %>% filter(status=="Aprobado") %>% count(mes) %>% ggplot(aes(x=mes,y=n,label=n))+geom_bar(stat = "identity",fill="lightblue")+geom_text())+labs(x=)
# probando con las fechas de created at y no de timestamp

prestamosporgestion<-prestamosporgestion %>% mutate(year=year(created_at))
prestamosporgestion<-prestamosporgestion %>% mutate(mes=month(created_at))
ggplotly(prestamosporgestion %>% filter(year==2022) %>% filter(status=="Aprobado") %>% count(mes) %>% ggplot(aes(x=mes,y=n,label=n))+geom_bar(stat = "identity",fill="lightblue")+geom_text())
